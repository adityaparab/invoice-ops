...
def build_invoice_graph(saver: BaseCheckpointSaver[str], nodes: InvoiceNodes) -> InvoiceGraph:
    ...
    builder.add_node("Match3Way", nodes.match_three_way)
    builder.add_node("Policy", nodes.policy)
    builder.add_node("Gate", nodes.gate)
    builder.add_node("AutoApprove", nodes.auto_approve)
    builder.add_node("ExceptionTriage", nodes.exception_triage)
    builder.add_node("HumanReview", nodes.human_review)
